...
class CustomPostgresMessageHistory(BaseChatMessageHistory):
    ...
    def messages(self) -> List[BaseMessage]:  # type: ignore
        ...
        message_model_list = (
            self.__db.query(MessagesDBModel)
            .filter(MessagesDBModel.window_id == self.window_id)
            .all()
        )
    ...
    def add_message(self, message: BaseMessage) -> None:
        ...
        message_db_model = MessagesDBModel(
            window_id=self.window_id, message=_message_to_dict(message)
        )
        try:
    ...
    def clear(self) -> None:
        ...
        try:
            self.__db.query(MessagesDBModel).filter(
                MessagesDBModel.window_id == self.window_id
            ).delete()
            self.__db.commit()
        except IntegrityError:
            self.__db.rollback()
